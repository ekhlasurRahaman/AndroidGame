$(function () {
  ...
  function create_single_button() {
    return $('<div id="begin" class="btn btn-success start"/>').text('Play now');
  }
  ...
  function create_resume_button() {
    var endings_remain = (unlocked_puzzles > 0 && unlocked_puzzles < 3);
      return $('<div id="resume" class="btn resume btn-success"/>').text(endings_remain ? 'Aim for a different ending'
                                                                               : 'Resume game');
  }
...
/* touchstart event */ 
  var touchable = document.querySelector("#content");

  if(touchable){ touchable.addEventListener("touchstart", function(e) 
   {
      var dx=  e.changedTouches[0].pageX;
      var dy= e.changedTouches[0].pageY;
      console.log(dx);
      console.log(dy);
      var width = window.innerWidth;
      var h = window.innerHeight;
      if(width >600){
          if(dx>75 && dx<119 && dy>259 && dy<308){
              move_player(-1, 0);  //left
          }else if(dx>134 && dx<180 && dy>258 && dy<304){
              move_player( 0,-1); //up
          }else if(dx>254 && dx<302 && dy>257 && dy<302){
              move_player( 1, 0);   //right
          }else if(dx>196 && dx<242 && dy>259 && dy<303){
              move_player( 0, 1);  //down
          }else if(dx>478 && dx<513 && dy>274 && dy<308){
              try_again();   //reload
          }else if(dx>527 && dx<556 && dy>274 && dy<304){
              player_uses_fork();  //Z
          }
      }else{
          if(dx>14 && dx<63 && dy>259 && dy<308){
              move_player(-1, 0);  //left
          }else if(dx>75 && dx<119 && dy>259 && dy<308){
              move_player( 0,-1); //up
          }else if(dx>193 && dx<241 && dy>259 && dy<308){
              move_player( 1, 0);   //right
          }else if(dx>135 && dx<183 && dy>259 && dy<308){
              move_player( 0, 1);  //down
          }else if(dx>423 && dx<453 && dy>259 && dy<308){
              try_again();   //reload
          }else if(dx>467 && dx<496 && dy>259 && dy<308){
              player_uses_fork();  //Z
          }
      }    
   }, false);}
